    ...
    for col_to_repair in columns_to_repair:
        ...
        if np.isnan(row[col_to_repair]):
            ...
            # Si la columna es nula y no la hemos visitado -> Intentamos repararla
                ...
                row[col_to_repair] = applyFix(
                    row, col_to_repair, visited_columns, trivial_fix
                )
...
def reconstructDf(df, trivial_fix=False):
    ...
    for index, row in df.iterrows():
        ...
        for null_colum in null_columns:
            df.loc[index, null_colum] = applyFix(
                row, null_colum, visited_columns=[], trivial_fix=trivial_fix
            )
...
def applyFix(row, col, visited_columns, trivial_fix=False):
    ...
    # COGS
        ...
        repaired = try_repair_column(
            ["Revenue", "Gross Profit"], row, visited_columns, trivial_fix
        )
    ...
    # Gross Profit
        ...
        # Fix 1
        repaired = try_repair_column(
            ["Revenue", "COGS"], row, visited_columns, trivial_fix
        )
        ...
        # Fix 2
        repaired = try_repair_column(
            ["Revenue", "Gross Profit Ratio"], row, visited_columns, trivial_fix
        )
    ...
    # Selling and Marketing Exp.
        ...
        if repaired:
            return (
                row["Selling, General and Administrative Exp."]
                - row["General and Administrative Exp."]
            )
        else:
            return np.nan
    ...
    # Acquisitions Net
        ...
        if repaired:
            return (
                row["Cash Used for Investing Activites"]
                - row["CAPEX"]
                - row["Purchases of Investments"]
                - row["Sales/Maturities of Investments"]
                - row["Other Investing Activites"]
            )

    # Minority Interest
    elif col == "Minority Interest":
        repaired = try_repair_column(
            ["Total Liabilities And Stockholders Equity", "Total Liabilities & Equity"],
            row,
            visited_columns,
            trivial_fix,
        )
    ...
    # Effect of Forex Changes on Cash
        ...
        if repaired:
            return (
                ...
                - row["Cash Provided by Operating Activities"]
                - row["Cash Used/Provided by Financing Activities"]
                - row["Cash Used for Investing Activites"]
            )
    ...
    # Other Comprehensive Income/Loss
        ...
        if repaired:
            return (
                ...
                - row["Other Total Stockholders Equity"]
            )
    ...
    # Selling, General and Administrative Exp.
        ...
        repaired = try_repair_column(
            ["Research and Development Exp.", "Operating Expenses", "Other Expenses"],
            row,
            visited_columns,
            trivial_fix,
        )
        ...
        if repaired:
            return (
                row["Operating Expenses"]
                - row["Research and Development Exp."]
                - row["Other Expenses"]
            )
    ...
    # Total Other Income Expenses (Gains)
        ...
        repaired = try_repair_column(
            ["Operating Income", "Income Before Tax"], row, visited_columns, trivial_fix
        )
    ...
    # COGS and Expenses
        ...
        repaired = try_repair_column(
            ["COGS", "Operating Expenses"], row, visited_columns, trivial_fix
        )
    ...
    # Income Tax Expense (Gain)
        ...
        repaired = try_repair_column(
            ["Income Before Tax", "Net Income"], row, visited_columns, trivial_fix
        )
    ...
    # EBITDA
        ...
        if repaired:
            return (
                row["Interest Expense (Gain)"]
                + row["Depreciation and Amortization"]
                + row["Operating Income"]
                + row["Total Other Income Expenses (Gains)"]
            )

        # Fix 2
        repaired = try_repair_column(
            ["EBITDA ratio", "Revenue"], row, visited_columns, trivial_fix
        )
    ...
    # Operating Income
        ...
        # Fix 2
        repaired = try_repair_column(
            ["Operating Income ratio", "Revenue"], row, visited_columns, trivial_fix
        )
        ...
        # Fix 3
        repaired = try_repair_column(
            ["Gross Profit", "Operating Expenses"], row, visited_columns, trivial_fix
        )
        ...
        # Fix 4
        repaired = try_repair_column(
            ["Income Before Tax", "Total Other Income Expenses (Gains)"],
            row,
            visited_columns,
            trivial_fix,
        )
    ...
    # Operating Income ratio
        ...
        repaired = try_repair_column(
            ["Operating Income", "Revenue"], row, visited_columns, trivial_fix
        )
    ...
    # Operating Expenses
        ...
        # Fix 1
        repaired = try_repair_column(
            ["Gross Profit", "Operating Income"], row, visited_columns, trivial_fix
        )
    ...
    # Cash at the Beginning of Period
        ...
        repaired = try_repair_column(
            ["Cash at the End of Period", "Net Change In Cash"],
            row,
            visited_columns,
            trivial_fix,
        )
    ...
    # Net Change In Cash
        ...
        if repaired:
            return (
                row["Cash at the End of Period"]
                - row["Cash at the Beginning of Period"]
            )
    ...
    # Income Before Tax
        ...
        # Fix 1
        repaired = try_repair_column(
            ["Net Income", "Interest Expense (Gain)"], row, visited_columns, trivial_fix
        )
        ...
        # Fix 2
        repaired = try_repair_column(
            ["Operating Income", "Total Other Income Expenses (Gains)"],
            row,
            visited_columns,
            trivial_fix,
        )
        ...
        # Fix 3
        repaired = try_repair_column(
            ["Income Before Tax Ratio", "Revenue"], row, visited_columns, trivial_fix
        )
    ...
    # Income Before Tax Ratio
        ...
        repaired = try_repair_column(
            ["Income Before Tax", "Revenue"], row, visited_columns, trivial_fix
        )
    ...
    # Cash and Cash Equivalents
        ...
        if repaired:
            return (
                row["Cash and Short-Term Investments"] - row["Short-Term Investments"]
            )
    ...
    # Cash Provided by Operating Activities
        ...
        if repaired:
            return (
                row["Net Change In Cash"]
                - row["Effect of Forex Changes on Cash"]
                - row["Cash Used/Provided by Financing Activities"]
                - row["Cash Used for Investing Activites"]
            )
        ...
        if repaired:
            return (
                row["Net Income"]
                + row["Depreciation and Amortization"]
                + row["Deferred Income Tax"]
                + row["Stock Based Compensation"]
                + row["Change in Working Capital"]
                + row["Other Non-Cash Items"]
            )

    # Total Stockholders Equity
    elif col == "Total Stockholders Equity":
        repaired = try_repair_column(
            ["Total Assets", "Total Liabilities"], row, visited_columns, trivial_fix
        )
    ...
    # Total Assets
        ...
        # Fix 1
        repaired = try_repair_column(
            ["Total Current Assets", "Total Non-Current Assets", "Other Assets"],
            row,
            visited_columns,
            trivial_fix,
        )
    ...
    # Total Liabilities & Equity
        ...
        repaired = try_repair_column(
            ["Total Liabilities", "Total Stockholders Equity", "Minority Interest"],
            row,
            visited_columns,
            trivial_fix,
        )
    ...
    # EPS Diluted
        ...
        repaired = try_repair_column(
            ["Net Income", "Weighted Average Shares Outstanding Diluted"],
            row,
            visited_columns,
            trivial_fix,
        )
        ...
        if repaired:
            return (
                row["Net Income"] / row["Weighted Average Shares Outstanding Diluted"]
            )

    # EBITDA ratio
    elif col == "EBITDA ratio":
        repaired = try_repair_column(
            ["EBITDA", "Revenue"], row, visited_columns, trivial_fix
        )
    ...
    # Net Income ratio
        ...
        repaired = try_repair_column(
            ["Net Income", "Revenue"], row, visited_columns, trivial_fix
        )
    ...
    # Net Income
        ...
        # Fix 1
        repaired = try_repair_column(
            ["Income Before Tax", "Income Tax expense (Gain)"],
            row,
            visited_columns,
            trivial_fix,
        )
        ...
        # Fix 2
        repaired = try_repair_column(
            ["Net Income ratio", "Revenue"], row, visited_columns, trivial_fix
        )
...
def calculateGrowthMetrics(df, columns):
    for column in columns:
        ...
        df[column + " growth 3 year average"] = (
            df[column + " growth year over year"]
            + df[column + " growth year over year"].shift(1)
            + df[column + " growth year over year"].shift(2)
        ) / 3
        df[column + " growth 5 year average"] = (
            df[column + " growth year over year"]
            + df[column + " growth year over year"].shift(1)
            + df[column + " growth year over year"].shift(2)
            + df[column + " growth year over year"].shift(3)
            + df[column + " growth year over year"].shift(4)
        ) / 5
        df[column + " growth 10 year average"] = (
            df[column + " growth year over year"]
            + df[column + " growth year over year"].shift(1)
            + df[column + " growth year over year"].shift(2)
            + df[column + " growth year over year"].shift(3)
            + df[column + " growth year over year"].shift(4)
            + df[column + " growth year over year"].shift(5)
            + df[column + " growth year over year"].shift(6)
            + df[column + " growth year over year"].shift(7)
            + df[column + " growth year over year"].shift(8)
            + df[column + " growth year over year"].shift(9)
            + df[column + " growth year over year"].shift(10)
        ) / 10
        df[column + " CAGR 3 years"] = (df[column].iloc[-1] / df[column].iloc[-4]) ** (
            1 / 3
        ) - 1
        df[column + " CAGR 5 years"] = (df[column].iloc[-1] / df[column].iloc[-6]) ** (
            1 / 5
        ) - 1
        df[column + " CAGR 10 years"] = (
            df[column].iloc[-1] / df[column].iloc[-11]
        ) ** (1 / 10) - 1
        years_since_start = df.index[-1] - df.index[0]
        df[column + " CAGR since start"] = (
            df[column].iloc[-1] / df[column].iloc[0]
        ) ** (1 / years_since_start) - 1
...
def fundamentalCalculator(df):
    ...
    fundamental_data["Book value"] = fundamental_data["Total Stockholders Equity"].sub(
        fundamental_data["Preferred Stock"], fill_value=0
    )
    fundamental_data["Net Interest Income"] = (
        fundamental_data["Interest Income "]
        - fundamental_data["Interest Expense (Gain)"]
    )

    # ==========================================        Ratios        ===============================================
    fundamental_data["Free Cash Flow ratio"] = (
        fundamental_data["Free Cash Flow"] / fundamental_data["Revenue"]
    )
    fundamental_data["Selling, General and Administrative Exp. Ratio"] = (
        fundamental_data["Selling, General and Administrative Exp."]
        / fundamental_data["Revenue"]
    )
    fundamental_data["Research and Development Exp. Ratio"] = (
        fundamental_data["Research and Development Exp."] / fundamental_data["Revenue"]
    )
    fundamental_data["Other Expenses Ratio"] = (
        fundamental_data["Other Expenses"] / fundamental_data["Revenue"]
    )
    fundamental_data["Net Interest Income Ratio"] = (
        fundamental_data["Net Interest Income"] / fundamental_data["Revenue"]
    )
    fundamental_data["Depreciation and Amortization Ratio"] = (
        fundamental_data["Depreciation and Amortization"] / fundamental_data["Revenue"]
    )

    # ==========================================        MÉTRICAS PER SHARE        ===============================================

    fundamental_data["Revenue per share"] = fundamental_data["Revenue"] / shares
    fundamental_data["Operating Income per share"] = (
        fundamental_data["Operating Income"] / shares
    )
    ...
    fundamental_data["Dividends per share"] = (
        fundamental_data["Dividends Paid"] / shares
    )

    # =======================================     MÉTRICAS DE SALUD FINANCIERA       ===============================================
    # Calculamos total debt
    fundamental_data["Total Debt"] = fundamental_data["Short-Term Debt"].add(
        fundamental_data["Long-Term Debt"], fill_value=0
    )

    # Calculamos el financial leverage
    fundamental_data["Financial leverage"] = (
        fundamental_data["Total Debt"] / fundamental_data["Total Stockholders Equity"]
    )

    # Calculamos el Cash to Debt Ratio
    fundamental_data["Cash to Debt Ratio"] = (
        fundamental_data["Cash and Cash Equivalents"] / fundamental_data["Total Debt"]
    )
    if np.inf in fundamental_data["Cash to Debt Ratio"].values:
        fundamental_data["Cash to Debt Ratio"] = fundamental_data[
            "Cash to Debt Ratio"
        ].replace(np.inf, 99)

    # Calculamos el Cash and Long Term Investemnts
    fundamental_data["Cash & Investments"] = (
        fundamental_data["Cash and Short-Term Investments"]
        + fundamental_data["Investments"]
    )
    # Calculamos el Cash and Long term investement to Debt Ratio
    fundamental_data["Cash & Investments to Debt Ratio"] = (
        fundamental_data["Cash & Investments"] / fundamental_data["Total Debt"]
    )
    if np.inf in fundamental_data["Cash & Investments to Debt Ratio"].values:
        fundamental_data["Cash & Investments to Debt Ratio"] = fundamental_data[
            "Cash & Investments to Debt Ratio"
        ].replace(np.inf, 99)

    # Calculamos Net Debt
    fundamental_data["Net Debt"] = (
        fundamental_data["Total Debt"]
        - fundamental_data["Cash and Short-Term Investments"]
    )
    # Calculamos Net Debt minus Investments
    fundamental_data["Net Debt w/Investments"] = (
        fundamental_data["Net Debt"] - fundamental_data["Investments"]
    )

    # Calculamos el Working Capital
    fundamental_data["Working Capital"] = (
        fundamental_data["Total Current Assets"]
        - fundamental_data["Total Current Liabilities"]
    )

    # Calculamos el Current Ratio
    fundamental_data["Current Ratio"] = (
        fundamental_data["Total Current Assets"]
        / fundamental_data["Total Current Liabilities"]
    )

    # Calculamos el Quick Ratio
    fundamental_data["Quick Ratio"] = (
        fundamental_data["Total Current Assets"] - fundamental_data["Inventory"]
    ) / fundamental_data["Total Current Liabilities"]

    # Calculamos el Debt to Equity
    fundamental_data["Debt to Equity"] = (
        fundamental_data["Total Current Liabilities"]
        / fundamental_data["Total Stockholders Equity"]
    )

    # Calculamos el Debt to Assets
    fundamental_data["Debt to Assets"] = (
        fundamental_data["Total Current Liabilities"] / fundamental_data["Total Assets"]
    )

    # Calculamos EBITDA to Net Debt
    fundamental_data["EBITDA to Net Debt"] = (
        fundamental_data["EBITDA"] / fundamental_data["Net Debt"]
    )

    # Calculamos el Interest Coverage
    fundamental_data["Interest Coverage"] = (
        fundamental_data["EBITDA"] / fundamental_data["Interest Expense (Gain)"]
    )
    # Ponemos valor 99 para reflejar que la situación es buena
    if np.inf in fundamental_data["Interest Coverage"].values:
        fundamental_data["Interest Coverage"] = fundamental_data[
            "Interest Coverage"
        ].replace(np.inf, 99)
    ...
    fundamental_data["Return on Equity"] = (
        fundamental_data["Net Income"] / fundamental_data["Total Stockholders Equity"]
    )

    # Calculamos el Return on Assets
    fundamental_data["Return on Assets"] = (
        fundamental_data["Net Income"] / fundamental_data["Total Assets"]
    )

    # Calculamos el ROIC
    fundamental_data["Tax Rate"] = (
        fundamental_data["Income Tax expense (Gain)"]
        / fundamental_data["Income Before Tax"]
    )
    fundamental_data["NOPAT"] = fundamental_data["Operating Income"] * (
        1 - fundamental_data["Tax Rate"]
    )
    fundamental_data["Invested Capital"] = fundamental_data[
        "Total Liabilities And Stockholders Equity"
    ].add(-fundamental_data["Total Current Liabilities"], fill_value=0)
    fundamental_data["Return on Invested Capital"] = (
        fundamental_data["NOPAT"] / fundamental_data["Invested Capital"]
    )

    # Calculamos el ROCE
    fundamental_data["Capital Employed"] = (
        fundamental_data["Total Assets"] - fundamental_data["Total Current Liabilities"]
    )
    fundamental_data["ROCE"] = (
        fundamental_data["Operating Income"] / fundamental_data["Capital Employed"]
    )

    # Calculamos el Operating Cash Flow
    fundamental_data["Operating Cash Flow"] = (
        fundamental_data["Operating Income"]
        + fundamental_data["Depreciation and Amortization"]
        - fundamental_data["Income Tax expense (Gain)"]
        + fundamental_data["Change in Working Capital"]
    )

    # Calculamos el CFROI
    fundamental_data["CFROI"] = (
        fundamental_data["Operating Cash Flow"] / fundamental_data["Capital Employed"]
    )

    # =======================================     MÉTRICAS DE REINVERSIÓN      ===============================================
    # Calculamos el Plowback Ratio
    fundamental_data["Plowback Ratio"] = (
        fundamental_data["Net Income"] - fundamental_data["Dividends Paid"]
    ) / fundamental_data["Net Income"]

    # Calculamos el Dividend & Repurchase / FCF
    fundamental_data["Dividend & Repurchase / FCF"] = (
        fundamental_data["Dividends Paid"]
        - fundamental_data["Common Stock Repurchased"]
        - fundamental_data["Common Stock Issued"]
    ) / fundamental_data["Free Cash Flow"]

    # Calculamos el Dividend & Repurchase / EBITDA
    fundamental_data["Dividend & Repurchase / EBITDA"] = (
        fundamental_data["Dividends Paid"]
        - fundamental_data["Common Stock Repurchased"]
        - fundamental_data["Common Stock Issued"]
    ...
    # =======================================     OTRAS MÉTRICAS      ===============================================
    fundamental_data["Asset Turnover Ratio"] = (
        fundamental_data["Revenue"] / fundamental_data["Total Assets"]
    )
    fundamental_data["Inventory Turnover"] = (
        fundamental_data["COGS"] / fundamental_data["Inventory"]
    )
    fundamental_data["Receivables Turnover"] = (
        fundamental_data["Revenue"] / fundamental_data["Accounts Receivable"]
    )
